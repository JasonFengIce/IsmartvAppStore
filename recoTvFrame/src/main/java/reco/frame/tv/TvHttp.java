...
public class TvHttp {
	...
	public HttpHandler<File> download(String url, AjaxParams params,
		...
		try {
			...
			handler = new HttpHandler<File>(appContext, httpClient, httpContext,
					callback, charset);
	...
	protected <T> void sendRequest(DefaultHttpClient client,
		...
		new HttpHandler<T>(appContext, client, httpContext, ajaxCallBack,
				charset).executeOnExecutor(executor, uriRequest);
